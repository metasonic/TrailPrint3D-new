...
def test_defaults_match_phase2_design() -> None:
    ...
    assert s.hexagon_orientation == "flat_top"
    assert s.buffer_mode == "percent"
    assert s.buffer_percent == 5.0
    assert s.buffer_absolute_mm == 3.0
    assert s.buffer_min_mm == 2.0
...
def test_buffer_percent_bounds() -> None:
    with pytest.raises(ValidationError):
        GenerateSettings(buffer_percent=-0.1)
    with pytest.raises(ValidationError):
        GenerateSettings(buffer_percent=100.1)
    GenerateSettings(buffer_percent=0.0)
    GenerateSettings(buffer_percent=100.0)


def test_buffer_mode_must_be_known() -> None:
    with pytest.raises(ValidationError):
        GenerateSettings(buffer_mode="custom")  # type: ignore[arg-type]


def test_hexagon_orientation_must_be_known() -> None:
    with pytest.raises(ValidationError):
        GenerateSettings(hexagon_orientation="diagonal")  # type: ignore[arg-type]
